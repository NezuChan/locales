...
export class EnUsCommandsMusic extends LanguagePiece {
    ...
    public NOW_PLAYING = (title: string, author: User | string, duration?: number | string): string => `Now playing: [${inlineCode(title)}] (${inlineCode(inlineCode(author instanceof User ? `<@${author.id}>` : `<@${author}>`))}) ${duration ? `[${duration}]` : ""}`;
    ...
    public TRACK_LOADED = (title: string, author: User | string, duration?: number | string): string => `Loaded track [${inlineCode(title)}] (${inlineCode(author instanceof User ? `<@${author.id}>` : `<@${author}>`)}) ${duration ? `[${duration}]` : ""}`;
    public PREVIOUS_TRACK_NOT_FOUND = (): string => "There is no previous track.";
    public PREVIOUS_TRACK = (title: string, author: User | string, duration?: number | string): string => `Now playing previous track: [${inlineCode(title)}] (${inlineCode(author instanceof User ? `<@${author.id}>` : `<@${author}>`)}) ${duration ? `[${duration}]` : ""}`;
    public REMOVED_TRACK = (title: string, author: User | string, duration?: number | string): string => `Removed track: [${inlineCode(title)}] (${inlineCode(author instanceof User ? `<@${author.id}>` : `<@${author}>`)}) ${duration ? `${duration ? `[${duration}]` : ""}` : ""}`;
